...
import live.bdfatafat.android.databinding.ActivitySignupBinding;
...
public class RegisterActivity extends AppCompatActivity {
    ...
    private void doSignUp() {
        ...
        apiInterface.signUp("signup", this.binding.edtMobileNo.getText().toString().trim(), this.binding.edtPassword.getText().toString().trim(), this.binding.edtUsername.getText().toString().trim()).enqueue(new Callback<RegisterResponse>() {
    ...
    private boolean validate() {
        if (this.binding.edtUsername.getText().toString().trim().isEmpty()) {
            ...
            return false;
        } else if (this.binding.edtMobileNo.getText().toString().trim().length() != 10) {
