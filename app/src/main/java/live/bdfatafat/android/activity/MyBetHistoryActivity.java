...
import live.bdfatafat.android.databinding.ActivityBetHistoryBinding;
...
public class MyBetHistoryActivity extends AppCompatActivity {
    ...
    private void getBetHistory(String searchKey) {
        ...
        ApiInterface apiInterface = (ApiInterface) ApiClient.getClient().create(ApiInterface.class);
        this.apiService = apiInterface;
        apiInterface.getWinHistory("bet_history", this.preference.getUserDetails().getMobile(), getIntent().getStringExtra("id"), "date", sorting, "10", "0", searchKey).enqueue(new Callback<WinHistoryResponse>() {
